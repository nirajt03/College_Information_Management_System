...
router.post('/signup', (request, response) => {
    ...
    const statement = `insert into hod (
...
router.post('/signin', (request, response) => {
    const { email, password } = request.body
        //const encryptedPassword = crypto.SHA256(password)
    const statement = `select hodId, name, phone from hod where email = '${email}' and password = '${password}'`
    db.query(statement, (error, hods) => {
        if (error) {
            response.send({ status: 'error', error: error })
        } else {
            if (hods.length == 0) {
                response.send({ status: 'error', error: 'hod does not exist' })
            } else {
                const hod = hods[0]
                const token = jwt.sign({ id: hod['hodId'] }, config.secret)
...
router.get('/hod', (request, response) => {
    const statement = `
        select * from hod;
    `
    db.query(statement, (error, data) => {
