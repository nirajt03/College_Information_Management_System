...
router.post('/signup', (request, response) => {
    ...
    const statement = `insert into principal (
...
router.post('/signin', (request, response) => {
    const { email, password } = request.body
        //const encryptedPassword = crypto.SHA256(password)
    const statement = `select principalId, name, phone from principal where email = '${email}' and password = '${password}'`
    db.query(statement, (error, principals) => {
        if (error) {
            response.send({ status: 'error', error: error })
        } else {
            if (principals.length == 0) {
                response.send({ status: 'error', error: 'principal does not exist' })
            } else {
                const principal = principals[0]
                const token = jwt.sign({ id: principal['principalId'] }, config.secret)
...
router.get('/principal', (request, response) => {
    const statement = `
